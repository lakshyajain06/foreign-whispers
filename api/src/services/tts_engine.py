...
def _synthesize_raw(tts_engine, text: str, wav_path: str, **kwargs) -> bytes | None:
...
def _synced_segment_audio(tts_engine, text: str, target_sec: float, work_dir, stretch_factor: float = 1.0, alignment_enabled: bool = True, **kwargs) -> tuple:
...
def text_file_to_speech(source_path, output_path, tts_engine=None, *, alignment=None, speaker_wav=None, speaker_voices=None):
    ...
    *alignment* overrides the module-level ``_ALIGNMENT_ENABLED`` flag.
    ...
    with tempfile.TemporaryDirectory() as synth_dir:
        def _do_synth(idx: int, text: str, speaker_label: str | None) -> tuple[int, bytes | None]:
            wav_path = str(pathlib.Path(synth_dir) / f"seg_{idx}.wav")
            
            current_speaker_wav = speaker_wav
            if speaker_voices and speaker_label and speaker_label in speaker_voices:
                current_speaker_wav = speaker_voices[speaker_label]
                
            kwargs = {}
            if current_speaker_wav:
                kwargs["speaker_wav"] = current_speaker_wav
                
            return idx, _synthesize_raw(engine, text, wav_path, **kwargs)
