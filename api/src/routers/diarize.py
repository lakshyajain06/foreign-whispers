...
from foreign_whispers.gender_classify import classify_speaker_gender
...
async def diarize_endpoint(video_id: str):
    ...
    try:
        ...
        stderr = exc.stderr.decode(errors="replace")[:500] if exc.stderr else str(exc)
    ...
    #DO GENDER CLASSIFY TOO
    
    gender = await asyncio.to_thread(
        classify_speaker_gender, str(audio_path), diar_segments
    )
    # Step 4: Cache result
    #   result = {"speakers": speakers, "segments": diar_segments}
    #   diar_path.write_text(json.dumps(result))
    result = {"speakers": speakers, "segments": diar_segments, "genders": gender}
    diar_path.write_text(json.dumps(result))
